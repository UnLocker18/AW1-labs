        ...
        <>
            ...
            <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="info" onClick={props.handleSubmit}>Confirm</Button>
                </Modal.Footer>
            </Modal>
        </>
    );
...
function TaskForm(props) {
    ...
    const formTimeField = <TimeField value={props.time}
        onChange={(ev, value) => props.handleTimeChange(ev, value)}
        input={
            <Form.Control className="max-width-5" />
        } 
    />   
        ...
        props.editMode < 0 ?
            <Form noValidate onSubmit={props.handleSubmit}>
                ...
                <Form.Group className="d-lg-flex mb-0">
                    <Col lg={8} className="d-flex align-items-center p-0 pr-lg-3 mb-2 mb-lg-0">
                        <Calendar className="mr-2" />
                        {formDeadline}
                    </Col>
                    <CustomFeedback validated={props.validated} breakpoint={!props.lg} isValid={props.date.isValid} >
            ...
            <ListGroup.Item className="edit-animation">
                <Form noValidate onSubmit={props.handleSubmit} className="d-flex align-items-center flex-column flex-md-row">
                    <Col lg={4} as="span" className="">
                        {formDescription}
                    </Col>
                    <Col lg={3} as="span" className="d-flex justify-content-center my-3 my-lg-0">
                        {formPrivate} {formImportant}
                    </Col>
                    <Col lg={5} as="span" className="d-flex justify-content-end align-items-center">
                        <Form.Control className="mr-3" type="date" onChange={(ev) => props.handleDateChange(ev)}
                                isInvalid={!props.validated ? false : !props.date.isValid}
                                isValid={!props.validated ? false : props.date.isValid}
                                value={props.date.value}
                        />
                        ...
                        <div className="d-flex align-items-center ml-3">
                            <X size={32} className="hover-scaleup cursor-pointer" onClick={() => props.editTask(-1)} />
                            <Check size={40} color="#17a2b8" className="hover-scaleup cursor-pointer" onClick={props.handleSubmit} />
                        </div>                        
                    </Col>
                </Form>
            </ListGroup.Item>
...
function CustomFeedback(props) {
    ...
    return(
        <div className={"margin-l-14 custom-invalid-feedback " +
            (!props.validated ? false : props.breakpoint && !props.isValid && "custom-is-invalid")
        }>
            {props.children}
        </div>
    );
