...
function Main(props) {
  ...
  const [date, setDate] = useState({ value: undefined, isValid: false });
  ...
  const editTask = tskID => {
    if (tskID >= 0) {
      ...
      const time = currentTask.date.hour() + ':' + currentTask.date.minute();
  ...
  const clearForm = () => {
    ...
    setDate({ value: undefined, isValid: false });
    setTime('00:00');
  };
